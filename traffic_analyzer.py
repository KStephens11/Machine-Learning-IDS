...
class TrafficAnalyzer:
    ...
    def get_prediction(self, data):
        columns = [
            "dst_port", "proto", "flow_duration", "tot_fwd_pkts", "tot_bwd_pkts", "total_fwd_pkt_len",
            "total_bwd_pkt_len",
        ...
        df = pandas.DataFrame([data[3:]], columns=columns)
        flow_info = data[:4]
        result = self.model.predict(df)
        result_label = self.label_encoder.inverse_transform(result)
        result_output = f"{str(flow_info):<10} : {str(result):<3} : {str(result_label):<10}"
